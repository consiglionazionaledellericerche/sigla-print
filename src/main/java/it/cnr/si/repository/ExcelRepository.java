...
public interface ExcelRepository extends CrudRepository<ExcelSpooler, Long> {
    ...
    @Query("select p.pgEstrazione from ExcelSpooler p, ParametriEnte e  where e.attivo = 'Y' and p.stato = 'S' and p.dtProssimaEsecuzione is null AND TRUNC(CURRENT_TIMESTAMP - p.duva) > Nvl(e.cancellaStampe,30)")
